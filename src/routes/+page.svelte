<script lang="ts">
	...
	// Editor import removed for dynamic loading
	...
	let EditorComponentClass = $state<any>(null);
	...
	let windowTitle = $derived(
		metadata.title ? `${metadata.title} - AppThere Loki Text` : 'AppThere Loki Text'
	);
	...
	async function handleSave() {
		...
		if (!path) {
			...
			// Normalize path
			path = path.startsWith('file://') ? decodeURIComponent(path.slice(7)) : path;
	...
	async function handleSaveAs(explicitFormat?: 'odt' | 'fodt') {
		...
		// Normalize path
		const normalizedPath = path.startsWith('file://') ? decodeURIComponent(path.slice(7)) : path;
	...
	async function handleExportEpub() {
		...
		try {
			...
			const fontDir = '/Users/kevin/.gemini/antigravity/scratch/appthere-loki/static/fonts';
			...
			const FONT_MAP: Record<string, string[]> = {
				...
				'Public Sans': ['PublicSans-Variable.ttf', 'PublicSans-Italic-Variable.ttf'],
				Newsreader: ['Newsreader-Variable.ttf', 'Newsreader-Italic-Variable.ttf'],
				'Cormorant Garamond': [
					'CormorantGaramond-Variable.ttf',
					'CormorantGaramond-Italic-Variable.ttf'
				],
				Geist: ['Geist-Variable.ttf'],
				'Bodoni Moda': ['BodoniModa-Variable.ttf', 'BodoniModa-Italic-Variable.ttf'],
			...
			await fileService.saveEpub(path, JSON.stringify(tiptapJson), stylesMap, metadata, fontPaths);

			// fileService handles writing if needed (e.g. Android content URIs) or backend does it
			addDebugLog('EPUB Export: Service call completed.');
	...
	async function handleOpen() {
		...
		if (isDirty) {
			const confirmed = await fileService.confirm(
	...
	async function handleOpenRecent(path: string) {
		...
		if (isDirty) {
			addDebugLog('handleOpenRecent: Document is dirty, asking confirmation...');
			const confirmed = await fileService.confirm(
				'You have unsaved changes. Are you sure you want to open another document?',
				{
					title: 'Open Recent',
	...
	async function openDocumentWithPath(path: string) {
		...
		let cleanedPath = path.startsWith('file://') ? decodeURIComponent(path.slice(7)) : path;

		console.log('openDocumentWithPath opening:', cleanedPath);
		syncStatus = 'Opening...';

		try {
			// Delegate entire opening logic to FileService which now handles content://
			const response = await fileService.openDocument(cleanedPath);
			console.log('open_document response:', response);

			const { content, styles, metadata: loadedMetadata } = response;
	...
	$effect(() => {
		// Auto-save session when document becomes dirty
		if (isDirty) {
			saveSession();
		}
	});
	...
	async function restoreSession() {
		...
		try {
			...
			await tick();
			if (view === 'editor' && !editorComponent) {
				console.warn('restoreSession: editorComponent still null after tick, retrying...');
				for (let i = 0; i < 5 && !editorComponent; i++) {
					await tick();
				}
			}

			if (editorComponent && session.content) {
				console.log('restoreSession: Calling loadWithStyles on editorComponent');
				...
				console.warn(
					'restoreSession: Skip content restore - editorComponent:',
					!!editorComponent,
					'content:',
					!!session.content
				);
	...
	onMount(() => {
		...
		addDebugLog('App: onMount started');

		const setup = async () => {
			try {
				console.log('App: Loading Editor component...');
				const mod = await import('$lib/Editor.svelte');
				EditorComponentClass = mod.default;
				console.log('App: Editor component loaded');
				addDebugLog('App: Editor component loaded');

				// Wait for components to be available in DOM if we're going to restore
				await tick();

				await restoreSession();
				console.log('App: restoreSession completed');
				addDebugLog('App: restoreSession completed');
			} catch (e) {
				console.error('App: Initialization failed', e);
				addDebugLog(`CRITICAL: Initialization failed: ${e}`);
			}
		};
...
{#if view === 'landing'}
	<LandingPage onNew={handleNewDocument} onOpen={handleOpen} onOpenRecent={handleOpenRecent} />
{:else}
		...
		<header class="app-header">
			...
			<div class="header-actions">
				<div class="menu-container">
					<button class="menu-btn" onclick={toggleMenu} title="Menu" aria-label="App Menu">
						<Menu size={20} />
					</button>
		...
		<div class="content-view">
			...
			{:else}
				<div
					...
					style="display: flex; justify-content: center; align-items: center; height: 100%; color: var(--icon-color);"
				>
					Loading Editor...
				</div>
			{/if}
...
<style>
	...
	.dirty-indicator {
		color: var(--text-color); /* Or a specific color like orange/red if preferred */
		font-size: 1.2rem;
		line-height: 0.5;
	}
