...
use common_core::colour_management::{
    BuiltInProfile, Colour, ColourContext, ColourSpace, DocumentColourSettings,
    IccProfileRef, IccProfileStore, SwatchLibrary, lookup_pantone,
};
...
fn cmyk_to_display_rgb_round_trip() {
    ...
    let colour = Colour::Cmyk { c: 0.2, m: 0.4, y: 0.1, k: 0.05, alpha: 0.75 };
    let result = ctx.convert(&colour);
    for (i, &ch) in result.iter().enumerate() {
        assert!((0.0..=1.0).contains(&ch), "channel {} out of range: {}", i, ch);
    }
    // Alpha must be preserved
    assert!((result[3] - 0.75).abs() < 1e-5, "alpha mismatch: {}", result[3]);
}
...
fn batch_matches_individual() {
    ...
    let colours: Vec<Colour> = (0..20).map(|i| {
        if i % 2 == 0 {
            Colour::Rgb { r: i as f32 / 20.0, g: 0.5, b: 0.3, a: 1.0 }
        } else {
            Colour::Cmyk { c: i as f32 / 40.0, m: 0.2, y: 0.1, k: 0.05, alpha: 1.0 }
        }
    }).collect();
    ...
    for (i, c) in colours.iter().enumerate() {
        ...
        for ch in 0..4 {
            assert!((batch[i][ch] - single[ch]).abs() < 1e-5,
                "colour {i}, channel {ch}: batch={} single={}", batch[i][ch], single[ch]);
        }
    }
}
...
fn swatch_pantone_integration() {
    ...
    let spot = Colour::Spot {
        ...
        cmyk_fallback: Box::new(Colour::Cmyk {
            c: 0.0, m: 0.91, y: 0.76, k: 0.06, alpha: 1.0,
        }),
    };
    let mut lib = SwatchLibrary::new();
    let _id = lib.add_colour("PANTONE 186 C", spot);
    let found = lib.find_by_name("pantone 186 c").expect("Should find case-insensitively");
...
fn colour_serde_round_trip() {
    let variants = vec![
        Colour::Rgb { r: 0.5, g: 0.3, b: 0.8, a: 1.0 },
        Colour::Cmyk { c: 0.1, m: 0.2, y: 0.3, k: 0.4, alpha: 0.9 },
        Colour::Lab { l: 50.0, a: 25.0, b: -30.0, alpha: 1.0 },
        Colour::Spot {
            name: "PANTONE 286 C".to_string(),
            tint: 0.8,
            lab_ref: [28.12, 15.34, -58.45],
            cmyk_fallback: Box::new(Colour::Cmyk {
                c: 1.0, m: 0.74, y: 0.0, k: 0.01, alpha: 1.0,
            }),
        },
        Colour::Linked { id: "swatch-abc123".to_string() },
    ];
...
fn document_colour_settings_serde_round_trip() {
    ...
    let recovered: DocumentColourSettings = serde_json::from_str(&json)
