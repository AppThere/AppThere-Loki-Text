<script lang="ts">
	...
	import { Plus, Trash2, X, CaseSensitive, AlignJustify, Layers } from 'lucide-svelte';
	...
	function loadStyle(style: BlockStyle) {
		...
		fontWeight = isNaN(weight) ? (style.fontWeight === 'bold' ? 700 : 400) : weight;
	...
	function saveStyle() {
		...
		const style: BlockStyle = {
			...
			fontSize,
			fontWeight:
				fontWeight === 700 ? 'bold' : fontWeight === 400 ? 'normal' : fontWeight.toString(),
			lineHeight,
			marginLeft,
			marginRight,
			textIndent,
	...
	let googleFontUrl = $derived.by(() => {
		...
		if (GOOGLE_FONTS.has(firstFamily)) {
			...
			if (fontWeight !== 400)
				weights.push(
					typeof fontWeight === 'string' ? parseInt(fontWeight) : (fontWeight as number)
				);
	...
	function normalizeUnit(value: string, type: 'length' | 'lineHeight' = 'length'): string {
	...
	>
		...
		>
			<header class="modal-header">
					...
					<div class="style-selector-container">
						...
						{:else}
							<div class="selector-wrapper">
								<select bind:value={selectedStyleId} class="style-select-header">
									{#each $styleRegistry as style}
										<option value={style.id}>{style.displayName || style.name}</option>
									{/each}
								</select>
								<div class="header-actions">
									<button class="icon-btn-header" onclick={startCreate} title="New Style">
										<Plus size={18} />
									</button>
									<button
				...
				<button class="header-close-btn" onclick={onClose} aria-label="Close">
					<X size={20} />
				</button>
			</header>
			...
			<div class="main-body">
				{#if isCreating && activeTab === 'general'}
					<div class="create-notice">
						<button class="cancel-link" onclick={cancelCreate}>Cancel new style</button>
					</div>
				{/if}
				...
				<div class="form-container">
					{#if activeTab === 'general'}
						<div class="form-group">
							...
							<input id="name" bind:value={newName} placeholder="e.g., Block Quote" />
						</div>
						<div class="form-row">
						...
						<div class="form-group">
							...
							<input id="desc" bind:value={newDesc} placeholder="Purpose of this style..." />
						</div>
						<div class="form-group">
							<label for="outlineLevel">Semantic Level (Outline Level)</label>
							<select id="outlineLevel" bind:value={outlineLevel}>
								<option value={undefined}>None (Body Text)</option>
								<option value={1}>Level 1 (Heading 1 / Title)</option>
								<option value={2}>Level 2 (Heading 2 / Slugline)</option>
								<option value={3}>Level 3 (Heading 3 / Character)</option>
							...
							<p class="field-help">Used for document navigation and Table of Contents.</p>
						</div>
					...
					{:else if activeTab === 'typography'}
						<div class="form-group">
							...
							<input list="fonts" id="fontFamily" bind:value={fontFamily} />
							<datalist id="fonts">
						...
						<div class="form-row">
							<div class="form-group grow">
								...
								<input
									id="fontSize"
									bind:value={fontSize}
									onblur={() => (fontSize = normalizeUnit(fontSize))}
								/>
							</div>
							<div class="form-group grow">
					...
					{:else if activeTab === 'paragraph'}
						...
						<div class="form-row">
							<div class="form-group grow">
								...
								<input
									...
									onblur={() => (marginTop = normalizeUnit(marginTop))}
								/>
							</div>
							<div class="form-group grow">
								...
								<input
									...
									onblur={() => (marginBottom = normalizeUnit(marginBottom))}
								/>
							</div>
						</div>
						<div class="form-row">
							<div class="form-group grow">
								...
								<input type="number" id="orphans" bind:value={orphans} min="1" max="10" />
							</div>
							<div class="form-group grow">
						...
						<div class="form-row">
							<div class="form-group grow">
								...
								<select id="breakBefore" bind:value={breakBefore}>
