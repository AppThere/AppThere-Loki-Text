<script lang="ts">
	...
	import { FileText, Plus, FolderOpen, Clock, File, Search, Trash2 } from 'lucide-svelte';
	...
	onMount(async () => {
		// Delay execution to isolate startup race conditions
		setTimeout(async () => {
			try {
	...
	async function confirmRemove(event: MouseEvent, doc: RecentDoc) {
		...
		const removeOption = await fileService.confirm(
					...
					<div class="recent-list">
						{#each recents as doc}
							<div
								...
								onclick={async () => {
									/* simplified handling for bisection */
									console.log('Recent clicked', doc.path);
									if (onOpenRecent) onOpenRecent(doc.path);
								}}
								role="button"
								tabindex="0"
								onkeydown={() => {}}
							>
								...
								<div class="doc-info">
									...
									<div class="doc-meta">
										{formatDate(doc.lastOpened)} • {doc.path.split('/').pop()}
									</div>
								</div>
								<button class="remove-btn" onclick={(e) => confirmRemove(e, doc)}
