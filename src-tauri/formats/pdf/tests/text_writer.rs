...
fn write_text_pdf_empty_blocks_fails_validation() {
    let font_bytes = match load_public_sans() {
        Some(b) => b,
        None => { eprintln!("[skip] Font unavailable"); return; }
    };
    let resolver = make_resolver_with_font(font_bytes);
    let metadata = Metadata { title: Some("Test".to_string()), ..Default::default() };
    let result = write_text_pdf(&[], &HashMap::new(), &metadata, &default_settings(), &resolver);
...
fn write_text_pdf_simple_paragraph_produces_pdf() {
    ...
    let metadata = Metadata { title: Some("Test Document".to_string()), ..Default::default() };
    let result = write_text_pdf(&blocks, &HashMap::new(), &metadata, &default_settings(), &resolver);
    assert!(result.is_ok(), "Simple paragraph should succeed: {:?}", result);
    let bytes = result.unwrap();
    // PDF files start with %PDF-
    assert!(bytes.starts_with(b"%PDF-"), "Output should be a PDF, got {:?}", &bytes[..8]);
}
...
fn write_text_pdf_mixed_blocks_succeed() {
    ...
    let metadata = Metadata { title: Some("Mixed Blocks".to_string()), ..Default::default() };
...
fn write_text_pdf_missing_title_still_exports() {
    ...
    assert!(result.is_ok(), "Missing title should not block export: {:?}", result);
}
